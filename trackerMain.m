function [results] = trackerMain(p, im, bg_area, fg_area, area_resize_factor, opticFlow)
...
%     [bg_hist, fg_hist] = updateHistModel(new_pwp_model, patch_padded(:,:,1), bg_area, fg_area, target_sz, p.norm_bg_area, p.n_bins, 1);
...
%             [likelihood_map] = getColourMap(im_patch_pwp(:,:,1), bg_hist, fg_hist, p.n_bins, 1);
...
%             [bg_hist, fg_hist] = updateHistModel(new_pwp_model, im_patch_bg(:,:,1), bg_area, fg_area, target_sz, p.norm_bg_area, p.n_bins, 1, bg_hist, fg_hist, p.learning_rate_pwp);
        end
